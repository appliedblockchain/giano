...
contract TestExecuteWithoutChainIdValidation is SmartWalletTestBase, TestPlus {
    function testExecuteBatch() public {
        ...
        GianoSmartWallet.Call[] memory calls = new GianoSmartWallet.Call[](2);
    ...
    function testExecuteBatch(uint256 r) public {
        ...
        unchecked {
            uint256 n = r & 3;
            GianoSmartWallet.Call[] memory calls = new GianoSmartWallet.Call[](n);
            ...
            if (_random() & 1 == 0) {
                MockGianoSmartWallet(payable(address(account))).executeBatch(_random(), calls);
            } else {
                account.executeBatch(calls);
            }
