...
contract ERC721AccountFactory {
    ...
    event AccountCreated(uint256 userId, ERC721Account.PublicKey publicKey, address account);

    function getUser(uint256 id) public view returns (User memory) {
        return users[id];
    }

    function createAccount(uint256 id, ERC721Account.PublicKey memory publicKey) public {
        ERC721Account account = new ERC721Account(publicKey);
        users[id] = User(id, publicKey, address(account));
        emit AccountCreated(id, publicKey, address(account));
    }
}
