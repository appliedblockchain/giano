...
/**
 * Server-side storage implementation example
 */
...
/**
 * Hybrid storage that tries localStorage first, then falls back to server
 */
export class HybridStorage implements GianoStorage {
  constructor(
  ...
  isAvailable(): boolean {
    return this.localStorage.isAvailable() || this.serverStorage.isAvailable();
  }

  // Provider session management
  async getCredentialId(): Promise<string | null> {
    const localValue = this.localStorage.getCredentialId();
    if (localValue) return localValue;

    const serverValue = await this.serverStorage.getCredentialId();
    if (serverValue && this.localStorage.isAvailable()) {
      this.localStorage.setCredentialId(serverValue);
    }
    return serverValue;
  }
  ...
  async setCredentialId(id: string): Promise<void> {
    if (this.localStorage.isAvailable()) {
      this.localStorage.setCredentialId(id);
    }
    await this.serverStorage.setCredentialId(id);
  }

  async getAccountAddress(): Promise<string | null> {
    const localValue = this.localStorage.getAccountAddress();
    if (localValue) return localValue;

    const serverValue = await this.serverStorage.getAccountAddress();
    if (serverValue && this.localStorage.isAvailable()) {
      this.localStorage.setAccountAddress(serverValue);
    }
    return serverValue;
  }
  ...
  async setAccountAddress(address: string): Promise<void> {
    if (this.localStorage.isAvailable()) {
      this.localStorage.setAccountAddress(address);
    }
    await this.serverStorage.setAccountAddress(address);
  }

  // Passkey management
  async getPasskeyId(): Promise<string | null> {
    const localValue = this.localStorage.getPasskeyId();
    if (localValue) return localValue;

    const serverValue = await this.serverStorage.getPasskeyId();
    if (serverValue && this.localStorage.isAvailable()) {
      this.localStorage.setPasskeyId(serverValue);
    }
    return serverValue;
  }
  ...
  async setPasskeyId(id: string): Promise<void> {
    if (this.localStorage.isAvailable()) {
      this.localStorage.setPasskeyId(id);
    }
    await this.serverStorage.setPasskeyId(id);
  }

  async getPublicKey(idHash: Hash): Promise<{ x: Hex; y: Hex } | null> {
    const localValue = this.localStorage.getPublicKey(idHash);
    if (localValue) return localValue;

    const serverValue = await this.serverStorage.getPublicKey(idHash);
    if (serverValue && this.localStorage.isAvailable()) {
      this.localStorage.setPublicKey(idHash, serverValue);
    }
    return serverValue;
  }
  ...
  async setPublicKey(idHash: Hash, coords: { x: Hex; y: Hex }): Promise<void> {
    if (this.localStorage.isAvailable()) {
      this.localStorage.setPublicKey(idHash, coords);
    }
    await this.serverStorage.setPublicKey(idHash, coords);
  }
