...
describe('PassKey', () => {
  ...
  describe('Use cases', () => {
    it('should validate passkey signature', async () => {
      ...
      const [pubKeyX, pubKeyY] = await helpers.getPublicKeyFromBytes(publicKey);
      ...
      const [sigX, sigY] = helpers.getMessageSignature(payload.response.signature);
      ...
      const response = await passkey.verifyPassKeySignature({ keyId: 'any', pubKeyX, pubKeyY }, sigX, sigY, helpers.bufToBn(data));
