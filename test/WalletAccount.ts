...
import { ERC1967Proxy__factory, TestCounter__factory, TestUtil__factory, WalletAccount__factory, WalletAccountFactory__factory } from '../typechain';
import type { TestCounter, TestUtil, WalletAccount } from '../typechain-types';
import { createAccount, createAccountOwner, createAddress, deployEntryPoint, getBalance, HashZero, isDeployed, ONE_ETH } from './testutils';
import { encodeUserOp, fillUserOpDefaults, getUserOpHash, packUserOp, signUserOp } from './UserOp';
...
describe('WalletAccount', function () {
  ...
  it('owner should be able to call transfer', async () => {
    ...
    await ethersSigner.sendTransaction({ from: accounts[0], to: account.address, value: ethers.parseEther('2') });
    await account.execute(accounts[2], ONE_ETH, '0x');
  });
  it('other account should not be able to call transfer', async () => {
    const { proxy: account } = await createAccount(ethers.provider.getSigner(), accounts[0], entryPoint);
    await expect(account.connect(ethers.provider.getSigner(1)).execute(accounts[2], ONE_ETH, '0x')).to.be.revertedWith('account: not Owner or EntryPoint');
  });
  ...
  describe('#executeBatch', () => {
    ...
    it('should allow transfer value', async () => {
      ...
      await ethersSigner.sendTransaction({ from: accounts[0], to: account.address, value: ethers.parseEther('2') });
      const rcpt = await account.executeBatch([target, counter.address], [ONE_ETH, 0], ['0x', counterJustEmit]).then(async (t) => await t.wait());
    ...
    it('should fail with wrong array length', async () => {
      const counterJustEmit = await counter.populateTransaction.justemit().then((tx) => tx.data!);
      await expect(account.executeBatch([counter.address, counter.address], [0], [counterJustEmit, counterJustEmit])).to.be.revertedWith('wrong array lengths');
    });
  });
  ...
  describe('#validateUserOp', () => {
    ...
    before(async () => {
      ...
      // cant use "WalletAccountFactory", since it attempts to increment nonce first
      const implementation = await new WalletAccount__factory(ethersSigner).deploy(entryPointEoa);
      const proxy = await new ERC1967Proxy__factory(ethersSigner).deploy(implementation.address, '0x');
      account = WalletAccount__factory.connect(proxy.address, epAsSigner);

      await ethersSigner.sendTransaction({ from: accounts[0], to: account.address, value: ethers.parseEther('0.2') });
