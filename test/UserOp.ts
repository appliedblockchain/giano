...
import type { IEntryPointSimulations } from '../typechain/contracts/core/EntryPointSimulations';
import type { EntryPoint } from '../typechain-type';
import { EntryPointSimulations__factory } from '../typechain-type';
...
export function encodeUserOp(userOp: UserOperation, forSignature = true): string {
  ...
  if (forSignature) {
    ...
    return defaultAbiCoder.encode(
      ['address', 'uint256', 'bytes', 'bytes', 'bytes32', 'uint256', 'uint256', 'uint256', 'bytes', 'bytes'],
      [
        packedUserOp.sender,
        packedUserOp.nonce,
