...
contract WalletFactory {
    ...
    function createAccount(address owner, uint256 salt) public returns (WalletAccount ret) {
        ...
        ret = WalletAccount(payable(new ERC1967Proxy{salt: bytes32(salt)}(address(accountImplementation), abi.encodeCall(WalletAccount.initialize, (owner)))));
    }
            ...
            Create2.computeAddress(
                ...
                keccak256(
                    abi.encodePacked(
                        ...
                        abi.encode(address(accountImplementation), abi.encodeCall(WalletAccount.initialize, (owner)))
                    )
                )
            );
    }
