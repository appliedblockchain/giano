...
library Secp256r1 {
     ...
     */
    function Verify(uint256 pubKeyX, uint256 pubKeyY, uint r, uint s, uint e) internal view returns (bool) {
        if (r == 0 || s == 0 || r >= nn || s >= nn) {
            /* testing null signature, otherwise (0,0) is valid for any message*/
            return false;
        }

        JPoint[16] memory points = _preComputeJacobianPoints(pubKeyX, pubKeyY);
    ...
    function _preComputeJacobianPoints(uint256 pubKeyX, uint256 pubKeyY) internal pure returns (JPoint[16] memory points) {
        ...
        points[1] = JPoint(pubKeyX, pubKeyY, 1); // u2
